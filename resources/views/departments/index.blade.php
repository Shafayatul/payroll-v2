...
<div class="row gutter30">
    <div class="col-md-4">
        <div class="block-section customvtab vtabs row">
            ...
            <ul id="department-list" class="nav nav-tabs tabs-vertical" data-toggle="tabs" ole="tablist">
                ...
                <li class="nav-item"><a class="nav-link {{$loop->iteration == 1? 'active':''}}" data-toggle="tab" onclick="openTab({{ $department->id }})" role="tab">{{ $department->name }}<span class="badge pull-right">7</span></a> </li>
                @endforeach
            </ul>
        </div>
    </div>
    <div class="col-md-8 tab-content">
        ...
        <div class="block-section tab-pane {{$loop->iteration == 1? 'active':''}} " id="tab{{$department->id}}" role="tabpanel">
            <h4 class="sub-header">{{$department->name}}<small> <a class="edit-toggle" href="#" data-toggle="tooltip" data-original-title="" title="" onclick="updateDepartment({{ $department->id }});">(Edit)</a> </small> <a href="#modal-delete-office" data-toggle="modal"> <i class="fas fa-trash pull-right" data-toggle="tooltip" title="" data-original-title="Delete this office"></i> </a>
            </h4>
            ...
            <form method="POST" action="{{ route('departments.update', $department->id)  }}" accept-charset="UTF-8" class="form-horizontal" style="display:none" id="department-{{$department->id}}"  novalidate="novalidate">
                @csrf
                @method('PUT')
